...
public class KBControl extends Control 
{
	...
	{
		final List<String> fileContent = FileHelper.getAllLinesInFile(Environment.getEnvironment().getExecutionPath() + "/" + filename);
		int totalTime = 0;
	...
	public static KBCommand createCommand(final Control owner, final Image image, final AbsDims startDims, final AbsDims finishDims, final int milliseconds)
	{
	...
	private void renderCommandTransition(final Graphics graphics,
			final KBCommand first,
			final KBCommand second,
