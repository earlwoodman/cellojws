...
public class Image implements Serializable {
	...
	private void initialize(final String filename_, final boolean load) {
		loaded = false;
		String executionPath = Environment.getEnvironment().getExecutionPath();
	...
	public void load() {
		...
		try {
			checkImageMapSize();
			System.out.println(filename + " - " + System.currentTimeMillis());
	...
	public static void checkImageMapSize() {
		...
		for (Image image : images.values()) {
			if (image.isLoaded()) {
				total += image.getHeight() * image.getWidth() * 4; // 4 bpp in
																	// 32-bit
																	// images
			}
		}
