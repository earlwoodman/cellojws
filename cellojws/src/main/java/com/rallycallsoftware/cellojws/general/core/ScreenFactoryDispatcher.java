...
public abstract class ScreenFactoryDispatcher implements CompletionCallback {
	...
	public void showCompanyLogo() {
		final Environment environment = Environment.getEnvironment();
		environment.getWindowManager().addWindow(getScreen(LogoScreen.class, new CompletionCallback() {
	...
	public <T> Screen getScreen(Class<?> class1, T payload) {
		...
		if (class1 == KenBurnsScreen.class) {
			...
			screen = new KenBurnsScreen((CompletionCallback) mapPayload.get("completionCallback"),
					(String) mapPayload.get("file"), (String) mapPayload.get("overlay"));
		}
	...
	public void showEventScreen(final Event event) {
		Image logo = null;
		...
		final Screen screen = getScreen(EventInfoScreen.class, payload);
		Environment.getEnvironment().getWindowManager().showPopup(screen);
	}
