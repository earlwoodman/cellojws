		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (Serializable.class.isAssignableFrom(fields[i].getType())) {
				try {
					...
					throw new RuntimeException(getPrettyPrintedStack(fields[i].getType().getName()), e);
				}
			}

			else {
				throw new RuntimeException(getPrettyPrintedStack(fields[i].getType().getName()).toString(),
