...
public class RealTime extends Thread implements Serializable
{
    ...
    public void pulse()
    {
        ...
        if( diffTick > 0 )
        {
            ...
            for( long i = 0; i < (long)minutesPassed; i++ )
            {
            	...
            	long oldYears = currentDateTime.getYear();
            	...
            	if( oldYears != currentDateTime.getYear() )
            	{
            		yearsIncreased(currentDateTime);
            	}
            }
    ...
    private void yearsIncreased(DateTime date_time) 
    {
    	synchronized(listenerLock)
    	{
	        for( RealTimeListener listener : listeners)
	        {
	            listener.yearsIncreased(date_time);
	        }
    	}
        resetTickCount();		
	}
