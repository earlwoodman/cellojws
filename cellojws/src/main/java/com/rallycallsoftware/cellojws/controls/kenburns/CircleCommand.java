...
public class CircleCommand implements KBCommand {
	...
	public BufferedImage getStartImage() {
		if (startImage == null) {
			startImage = scaleImage(
					getImage().getBufferedImage().getSubimage(start.left, start.top, start.getAbsWidth(),
							start.getAbsHeight()),
	...
	public void setImage(Image image) {
		...
		start = new AbsDims((imageWidth - viewWidth) / 2, 0, (imageWidth - viewWidth) / 2 + viewWidth, viewHeight);
	...
	public void render(final Graphics graphics, final float completePct, final AbsDims screenDims) {
		...
		midPoint.x = (int) (radius * Math.cos(completePct * 2 * Math.PI))
				+ getImage().getBufferedImage().getWidth() / 2;
		midPoint.y = (int) (radius * Math.sin(completePct * 2 * Math.PI))
				+ getImage().getBufferedImage().getHeight() / 2;
