		...
		if (!wrap) {
			...
			if (getJustification() == Justification.Left) {
				...
				if (getJustification() == Justification.Center) {
					graphics.drawText(text, dims.left + (width - textWidth > 0 ? width - textWidth : 0) / 2,
							centeredTop, dims.getAbsWidth());
					// WorkerLog.info(text);
				}
			...
			// We will also ignore scroll bars and just print as much text as
			// will fit.
