...
import com.rallycallsoftware.cellojws.special.TextTooltip;
...
public class ListBox extends Control {
	...
	public static final int PADDING = Environment.getEnvironment().quarterInch();
	...
	protected final static int SCROLLBAR_WIDTH = 20;
	...
	/**
	 * Denotes that a selection in the box is always required. (i.e. can't
	 * unselect)
	 */
	...
	public ListBox(final AbsDims dims, final List<String> captions_, final CommandToken<?> token,
		...
		setTooltip(new TextTooltip(this));

		noEntriesTop = hasHeaders() ? contents.getRowHeight() + getStdGap() : 0;
		noEntriesWidth = WindowManager.getTextWidth(noEntriesMsg, getFontInfo());

	}

	private int getItemsVisibleExcludeHeader() {
	...
	protected int getRowHeight() {
		...
		if (rowHeightByContents > 0) {
			...
			return rowHeightByContents + leftoverSpace / rowCount;
		}
	...
	public void reset() {
		...
		synchronized (this) {
			if (getItemsVisibleExcludeHeader() < contents.countLines() && getItemsVisibleExcludeHeader() > 0) {
				final AbsDims verticalScrollBarDims = new AbsDims(dims.getAbsWidth() - SCROLLBAR_WIDTH - 1, 1,
						dims.getAbsWidth() - 1, dims.getAbsHeight() - 1);
			...
			if (multiSelect) {
				...
				if (selected.contains(item)) {
					if (!selectionRequiredWhenSelectable) {
						// Autoboxing won't work here since remove() is
				...
				if (!clearing) {
					synchronized (this) {
						if (item > contents.countLines() - 1) {
							selected.add(contents.countLines() - 1);
						} else {
							selected.add(item);
						}
					}
	...
	protected int getItemByVerticalCoord(final int y) {
		final int item = (y - (hasHeaders() ? getRowHeight() : 0)) / getRowHeight()
				+ (verticalScrollBar != null && !overrideScrollBar ? verticalScrollBar.getTopIndex() : 0);
	...
	private void renderEntries(Graphics graphics, final AbsDims dims) {
		...
		if (contents.countLines() == 0) {
			graphics.drawText(noEntriesMsg, getScreenDims().left + (getScreenDims().getAbsWidth() - noEntriesWidth) / 2,
			...
			for (row = startPoint; row < max; row++) {
				...
				if (contents.countLines() > row) {
					...
					if (selected.contains(row) || (highlighted != null && highlighted.contains(row))) {
						highlight = true;
					}
	...
	private void renderHeader(Graphics graphics, final AbsDims dims) {
		...
		if (hasHeaders()) {
			...
			graphics.drawRect(new AbsDims(dims.left + 2, dims.top + getRowHeight(),
					dims.right - (verticalScrollBar != null ? SCROLLBAR_WIDTH : 0) - 2, dims.top + getRowHeight()));
		}
	}
	...
	private void renderWatermark(Graphics graphics, final AbsDims dims) {
		if (watermark != null && !watermark.isEmpty()) {
			...
			if (watermarkRenderLeft == -1 || watermarkRenderTop == -1) {
				...
				watermarkRenderLeft = (dims.getAbsWidth()
						- graphics.getTextWidth(watermark, environment.getWatermarkFontInfo())) / 2 + dims.left;
			}
	...
	protected AbsDims drawSingleRow(final Graphics graphics, final int startPoint, final int row,
			final boolean highlight, final List<? extends ListBoxItem> wholeLine, final int highlightColumn,
		...
		if (wholeLine != null) {
			...
			for (final ListBoxItem item : wholeLine) {
				count++;
				graphics.setHighlight(false);
				if (highlight // Someone set the highlight condition for this
								// row
						|| (count == highlightColumn && sortable) // Someone
																	// wants a
																	// specific
																	// column
																	// highlighted
						|| (count == 0 && highlightColumn == -1 && hasHeaders() && sortable)) // Header
																								// row
																								// is
																								// highlighted
				{
	...
	private AbsDims drawEntry(final Graphics graphics, final int startPoint, final int drawnRow, final int drawnColumn,
			final ListBoxItem item, final boolean isHeader) {
		...
		if (colLocs != null) {
			...
			final int left = (dims.left + getStdGap())
					+ (int) ((drawnColumn > 0 ? colLocs.get(drawnColumn - 1) : 0) * workingAreaWidth);// *
																										// drawnColumn;
			...
			if (colWidths.size() > 0) {
				final int scrollBarWidth = (verticalScrollBar != null && !overrideScrollBar
						? verticalScrollBar.getDimensions().getAbsWidth() : 0);
			...
			if (widthMax > 0) {
				...
				final AbsDims ret = new AbsDims(leftDraw, topCentered, widthMax + leftDraw,
						topCentered + WindowManager.getTextHeight(graphics.getFontInfo()));
				return ret;
			}
		}
	...
	private int determineTopToDraw(final Graphics graphics, final int startPoint, final int drawnRow,
			final AbsDims dims, final ListBoxItem item, final boolean isHeader) {
	...
	private int determineLeftDraw(ListBoxItem item, final int left, final String justification, final Graphics graphics,
			final int widthMax) {
		int leftDraw = left;
	...
	public synchronized void addContents(final List<? extends ListBoxItem> captions) {
		...
		/*
		 * for( final ListBoxItem item : captions ) { if( item instanceof
		 * Control ) { addControl((Control)item); } }
		 */
	}
	 ...
	 * 
	 * Works for both single and multiselect. Does not clear if multiselect is
	 * on.
	 * 
	 * @param item
	 */
	...
	public void setSelectionRequiredWhenSelectable(boolean selectionRequiredWhenSelectable) {
	...
	public void setTooltipByPosition(final int y) {
		...
		if (contents.getHeaders() != null) {
			...
			if (y < getRowHeight()) {
				...
				((TextTooltip) getTooltip()).setText(tooltip.toString());
				return;
			}
		}
		...
		if (item >= 0 && item < contents.countLines()) {
			...
			((TextTooltip) getTooltip()).setText("");
		}
	}
	...
	public void dragRender(Graphics graphics, boolean mousedown, AbsDims nonDraggingScreenDims, DragInfo dragging) {
		...
		graphics.setFontInfo(environment.getListBoxFontInfo());
		final AbsDims rowDims = drawSingleRow(graphics, determineStartPoint(), row, false, contents.getContentsAt(row),
				-1, false);
		...
		select(determineStartPoint() + row);
	}
