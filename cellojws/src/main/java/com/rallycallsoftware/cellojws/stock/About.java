...
public class About extends Screen {
	...
	public About() {
		...
		close = new BasicButton(SmallButtonType.getInstance(), "Close", closeToken,
				new Point((dims.getAbsWidth() - 125) / 2, dims.getAbsHeight() - 60));

		addControl(close);

		final Image logoImg = ImageFactory.getImage("logo/HockeyBrass.png");
		final AbsDims logoDims = new AbsDims((dims.getAbsWidth() - 300) / 2, 95, (dims.getAbsWidth() - 300) / 2 + 300,
				95 + 103);
		...
		final AbsDims programmingDims = new AbsDims(column1Left, row1Top, column1Right, row1Bottom);
		programming = new Label(programmingDims, "Programming By:");
		addControl(programming);

		final AbsDims graphicsDims = new AbsDims(column1Left, row2Top, column1Right, row2Bottom);
		graphics = new Label(graphicsDims, "Graphics By:");
		addControl(graphics);

		final AbsDims musicDims = new AbsDims(column1Left, row3Top, column1Right, row3Bottom);
		music = new Label(musicDims, "Music By:");
		addControl(music);

		final AbsDims programmingNameDims = new AbsDims(column2Left, row1Top, column2Right, row1Bottom);
		programmingName = new Label(programmingNameDims, "Earl Woodman");
		addControl(programmingName);

		final AbsDims graphicsNameDims = new AbsDims(column2Left, row2Top, column2Right, row2Bottom);
		graphicsName = new Label(graphicsNameDims, "Judy Mesh");
		addControl(graphicsName);

		final AbsDims musicNameDims = new AbsDims(column2Left, row3Top, column2Right, row3Bottom);
